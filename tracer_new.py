...
class Repository:
    ...
    def _create_stub_native_handler(self, target: TraceTarget, decorate: bool) -> str:
        if target.flavor == "objc":
            ...
            args_count_str = "%s"%(state["index"] - 2)
...

{
  ...
  onEnter(log, args, state) {
    if (Process.enumerateThreads()[0].id != Process.getCurrentThreadId()) {
        return
    }
    function get_args_types(symbol_str) {
        
        let addr = Module.findExportByName(null, '_get_method_argument_types')
        let get_method_argument_types  = new NativeFunction(
            addr,
            'pointer', // Return type
            ['pointer'] // Argument types
        );
        const ns_symbol_str = ObjC.classes.NSString.stringWithString_(symbol_str);
        let types = get_method_argument_types(ns_symbol_str)
        let ns_types = new ObjC.Object(types)
        const count = ns_types.count();
        // 创建一个空的 JavaScript 数组
        const jsArray = [];
        // 遍历 NSArray
        for (let i = 0; i < count; i++) {
            // 获取 NSArray 中的元素
            const element = ns_types.objectAtIndex_(i);
            // 将元素转换为 JavaScript 字符串并添加到 jsArray 中
            jsArray.push(element.toString());
        }
        return jsArray
    }
    function get_register(_this , types , index) {
        
        let res = ""
        const type = types[index]
        if (type == "@" || type == "#" || type == ":" || type == "*" ||
            type == "i" || type == "l" || type == "q" || type == "L" ||
            type == "Q"
            ) {
            // x
            res += ("x"+ `${_this.integer}`)
            _this.integer += 1
        }else if (type == "c" || type == "B" || type == "s" || type == "C" || type == "I" ) {
            // w
            res += ("x"+ `${_this.integer}`) // frida context.register only has x_reg
            _this.integer += 1
        }else if (type == "f") {
            // s
            res += ("s"+ `${_this.float}`)
    ...
    let args_count = %(args_count_str)s
    let method_name = "%(display_name)s"
    let _args = []
    let types = get_args_types(method_name)
    // log("types : " + types)

    let valid_args = []
    this.integer = 0
    this.float = 0
    for (let i = 0; i <  args_count+2; i++) {
        if (this.integer < 8) {
            const reg = get_register(this ,types , i)
            const val1 = this.context[reg].toString()
            const val2 = args[i].toString()
            // log(`${i} ${reg} val1 :${val1} , val2 : ${val2}` + (val1 != val2 ? "      X" : "") )
            _args.push(val1)
            valid_args.push(val1)
        }else {
            _args.push(args[i])
        }
    }
    ...
    let addr = Module.findExportByName(null, '_method_arg_str_desc')

    const method_arg_str_desc = new NativeFunction(
        addr,
        'pointer', // Return type
        ['pointer', 'pointer'] // Argument types
    );
    
    const ns_method_name = ObjC.classes.NSString.stringWithString_(method_name);

    let arr = ObjC.classes.NSMutableArray.new()
    for (let i = 2; i <  valid_args.length; i++) {
        const ns_val = ObjC.classes.NSString.stringWithString_(valid_args[i].toString());
        arr.addObject_(ns_val)
    }
    let res = method_arg_str_desc(ns_method_name , arr)
    let res_args = [args[0] , args[1]]
    let ns_res = new ObjC.Object(res)
    for (let i = 0; i <  ns_res.count(); i++) {
        let val = ns_res.objectAtIndex_(i)
        const js_str = (new ObjC.Object(val)).toString()
        res_args.push(js_str)
    }
    args.splice(0, res_args.length, ...res_args.slice(0, res_args.length));
    log(%(log_str)s);
  },
  ...
  onLeave(log, retval, state) {
    ...
    let method_name = "%(display_name)s"
    let addr = Module.findExportByName(null, '_method_return_str_desc')

    const method_return_str_desc = new NativeFunction(
        addr,
        'pointer', // Return type
        ['pointer', 'pointer'] // Argument types
    );
    ...
    const ret_ptr = ObjC.classes.NSString.stringWithString_(retval.toString());
    const res = method_return_str_desc(ns_method_name , ret_ptr)
    const ns_res = (new ObjC.Object(res)).toString()
    log("-> " + (ns_res.length > 0 ? `${ns_res} ` : "") + method_name)
  }
}
""" % {
            "display_name": target.display_name,
            "log_str": log_str,
            "args_count_str": args_count_str,
        }
